...
def train(epoch_num,model, ad_net, grl,ad_net_m,grl_m,ad_net_t, grl_t,ad_net_mt,grl_mt,optimizer, loss_fn, dataloader, metrics, params,logger):
    ...
    #trade_off = 0.2 * (1.0 -np.exp(-1 * 10 * epoch_num/400.0)) / (1.0 + np.exp(-1 * 10 * epoch_num / 400.0) )
    #trade_off=0.01
    trade_off=0.1
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, (data_batch, labels_batch) in enumerate(dataloader):
            ...
            #bsp loss
            sigma = Loss.BSP(fc7_feature)
            sigma_loss = sigma * 0.01
            ...
            loss = loss_bag['ls_CE'] + (transfer_loss + sigma_loss) * trade_off 
...
if __name__ == '__main__':
    ...
    if 'HCN' in params.model_version:
        ...
        #3D Online 1536
        # N2B 2048
        ad_net = HCN.AdversarialNetwork(6144) #26112 
        grl_m = HCN.GRL()
        ad_net_m = HCN.AdversarialNetwork(51)
        grl_t = HCN.GRL()
        ad_net_t = HCN.AdversarialNetwork(51)
        grl_mt = HCN.GRL()
        ad_net_mt = HCN.AdversarialNetwork(51)
