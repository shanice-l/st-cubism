...
def evaluate(model, loss_fn, dataloader, metrics, params,logger):
    ...
    for data_batch, labels_batch, rl_label in dataloader:
        ...
        output_batch,_,feature = model(data_batch.float())
        ...
        out_vis=list(np.array(output_batch.cpu()))
        label_vis=list(np.array(labels_batch.cpu()))

        for label in label_vis:
    ...
    print(len(label_out))
    print(len(data_out))
    #np.save('/opt/data5/New/Multi_task/fuse/label.npy',label_out)
    '''
    # compute mean of all metrics in summary
...
def train_and_evaluate(model,ad_net,grl, ad_net_mp, grl_mp ,train_dataloader, val_dataloader, optimizer,
    ...
    for epoch in range(params.start_epoch,params.num_epochs):
        ...
        train_metrics,train_confusion_meter = train(model,epoch,ad_net, grl,ad_net_mp,grl_mp, optimizer, loss_fn, train_dataloader, metrics, params,logger)
...
if __name__ == '__main__':
    ...
    if 'HCN' in params.model_version:
        ...
        ad_net = HCN.AdversarialNetwork(512) #adjustable
        ad_net_mp = HCN.AdversarialNetwork(1024) #adjustable
        #ad_net_m = HCN.AdversarialNetwork(512) #adjustable
