...
class Feeder(torch.utils.data.Dataset):
    ...
    def concateData(self):
        if self.data_path_target != 'None' :
            ...
            self.data = np.concatenate((self.data_source[:,:,:,:,:,None] , np.zeros((self.data_source.shape))[:,:,:,:,:,None]),axis = 5)
    ...
    def coordinate_transfer(self):
        ...
        if self.origin_transfer == 2:
            ...
            origin_source = data_numpy_source[:, :, :, 2, :]
            ...
            origin_source = data_numpy_source[:, :, :, 2, 0]
            ...
            origin_source = data_numpy_source[:, :, :, 2, 1]
            data_numpy_source = data_numpy_source - origin_source[:, :, :, None, None]
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.crop_resize:
            tmp0 = tools.random_get_frame(data_numpy[:,:,:,:,0], valid_frame_num_source, self.p_interval, self.window_size)
            if self.data_path_target != 'None':
