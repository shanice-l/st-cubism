...
def train(model, ad_net, grl,ad_net_m,grl_m,optimizer, loss_fn, dataloader, metrics, params,logger):
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, (data_batch, labels_batch) in enumerate(dataloader):
            ...
            loss_ad = Loss.JAN([feature.narrow(0,0,feature.size(0)//2),softmax_out.narrow(0,0,softmax_out.size(0)//2)],[feature.narrow(0,feature.size(0)//2,feature.size(0)//2),softmax_out.narrow(0,feature.size(0)//2,softmax_out.size(0)//2)])
...
def train_and_evaluate(model,ad_net,grl,ad_net_m,grl_m ,Myacc,train_dataloader, val_dataloader, optimizer,
    ...
    for epoch in range(params.start_epoch,params.num_epochs):
        ...
        train_metrics,train_confusion_meter = train(model, ad_net, grl,ad_net_m,grl_m,optimizer, loss_fn, train_dataloader, metrics, params,logger)
...
if __name__ == '__main__':
    ...
    if 'HCN' in params.model_version:
        ...
        grl = HCN.GRL()
        ad_net = HCN.AdversarialNetwork(256)
        grl_m = HCN.GRL()
        ad_net_m = HCN.AdversarialNetwork(256)
    ...
    ,{'params':filter(lambda p: p.requires_grad, ad_net_m.parameters()),'lr':1e-3}]
    #print(para_list)
    ...
    if params.mode == 'train' or params.mode == 'load_train':
        ...
        train_and_evaluate(model,ad_net, grl,ad_net_m,grl_m,Myacc,train_dl, test_dl, optimizer, loss_fn, metrics, params,
