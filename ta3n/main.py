...
def main():
	...
	data_path_prefix = "xxx"
	label_path_prefix = "xxx"
	num_frame_prefix = "xxx"
	...
	#val_set = TSNDataSet("", args.val_list, num_dataload=num_val, num_segments=val_segments,
						 ...
						 #test_mode=True,
						 #)
	val_set = feeder.Feeder(
			os.path.join(data_path_prefix, "NTU_CV_val_data.npy"),
			os.path.join(label_path_prefix, "NTU_CV_val_label.pkl"),
			os.path.join(num_frame_prefix, "NTU_CV_val_num_frame.npy"),
			window_size=window_size,
			origin_transfer=0,
			crop_resize=True,
			p_interval=[0.95])
	...
	if not args.evaluate:
		#source_set = TSNDataSet("", args.train_source_list, num_dataload=num_source_train, num_segments=args.num_segments,
								...
								#test_mode=True,
								#)
		source_set = feeder.Feeder(
				os.path.join(data_path_prefix, "PKU_whole_data.npy"),
				os.path.join(label_path_prefix, "PKU_whole_label.pkl"),
				os.path.join(num_frame_prefix, "PKU_whole_num_frame.npy"),
				window_size=window_size,
				origin_transfer=0,
				crop_resize=True,
				p_interval=[0.5, 1])
		...
		#target_set = TSNDataSet("", args.train_target_list, num_dataload=num_target_train, num_segments=args.num_segments,
								...
								#test_mode=True,
								#)
		target_set = feeder.Feeder(
				os.path.join(data_path_prefix, "NTU_CV_train_data.npy"),
				os.path.join(label_path_prefix, "NTU_CV_train_label.pkl"),
				os.path.join(num_frame_prefix, "NTU_CV_train_num_frame.npy"),
				window_size=window_size,
				origin_transfer=0,
				crop_resize=True,
				p_interval=[0.5, 1])
...
def train(num_class, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short, alpha, beta, gamma, mu):
	...
	for i, ((source_data, source_label),(target_data, target_label)) in data_loader:
		...
		if source_data.size(0) % gpu_count != 0:
			...
			source_data_dummy = torch.zeros((gpu_count - source_data.size(0) % gpu_count,) + source_data.shape[1:])
			source_data = torch.cat((source_data, source_data_dummy))
		if target_data.size(0) % gpu_count != 0:
			...
			target_data_dummy = torch.zeros((gpu_count - target_data.size(0) % gpu_count,) + target_data.shape[1:])
			target_data = torch.cat((target_data, target_data_dummy))
...
def validate(val_loader, model, criterion, num_class, epoch, log):
	...
	for i, (val_data, val_label) in enumerate(val_loader):
		...
		if batch_val_ori < args.batch_size[2]:
			...
			val_data_dummy = torch.zeros((args.batch_size[2] - batch_val_ori,) + val_size_ori[1:])
		...
		if val_data.size(0) % gpu_count != 0:
			...
			val_data_dummy = torch.zeros((gpu_count - val_data.size(0) % gpu_count,) + val_data.shape[1:])
			val_data = torch.cat((val_data, val_data_dummy))
