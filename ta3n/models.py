...
class VideoModel(nn.Module):
	def __init__(self, num_class, baseline_type, frame_aggregation, modality,
		...
		self._hcn = HCN(in_channel,
				num_joint,
				num_person,
				out_channel,
				window_size,
				num_class)
		self._adaptor = nn.Linear(2048, 2048)
	...
	def forward(self, hcn_input_source, hcn_input_target, beta, mu, is_train, reverse):
		num_segments = self.train_segments if is_train else self.val_segments
		#input_source = self._adaptor(self._hcn(hcn_input_source, nb_segments=num_segments))
		#input_target = self._adaptor(self._hcn(hcn_input_target, nb_segments=num_segments))
		input_source = self._hcn(hcn_input_source, nb_segments=num_segments)
		input_target = self._hcn(hcn_input_target, nb_segments=num_segments)
...
class HCN(nn.Module):
	...
	def forward(self, x, nb_segments=5, target=None):
		N, C, T, V, M = x.size()  # N0, C1, T2, V3, M4
		#print("Node 3", N, C, T, V, M)
		motion = x[:,:,1::,:,:]-x[:,:,0:-1,:,:]
		...
		#print("Node 1: {:}".format(out.shape))
		out = out.permute(0, 1, 3, 4, 2).contiguous()
		batch_size, _, _, _, time = out.shape
		out = torch.reshape(out, (batch_size, -1, time))
		out = F.interpolate(out, size=nb_segments, mode="linear").contiguous()\
				.permute(0, 2, 1).contiguous()
		#print("Node 2: {:}".format(out.shape))
		return out
		# max out logits
...
#
		#self._hcn = HCN(in_channel,
				...
				#window_size,
				#num_class)
		#self._video_model = VideoModel(num_class, baseline_type, frame_aggregation, modality,
				#train_segments, val_segments,
				#base_model, path_pretrained, new_length,
				#before_softmax,
				#dropout_i, dropout_v, use_bn, ens_DA,
				#crop_num, partial_bn, verbose, add_fc, fc_dim,
				#n_rnn, rnn_cell, n_directions, n_ts,
				#use_attn, n_attn, use_attn_frame,
				#share_params)
#
	#def forward(self, input_source, input_target, beta, mu, is_train, reverse):
		#source_feature = self._hcn(input_source)
		#target_feature = self._hcn(input_target)
		#return self._video_model(source_feature, target_feature, beta, mu, is_train, reverse)
