...
parser.add_argument('--dataset_name', default='kinetics', help="dataset name ") # NTU-RGB-D-CS,NTU-RGB-D-CV,kinetics
...
def train(model,ad_net, grl,ad_net_mp,grl_mp, optimizer, loss_fn, dataloader, metrics, params,logger):
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, (data_batch, labels_batch) in enumerate(dataloader):
            ...
            output_batch,feature,out_mp = model(data_batch.float(),target=labels_batch)
            #print(out_mp.size())
            #tsne
            '''
            out_batch = output_batch.data
            data_out=list(np.array(out_batch))
            for data in data_out:
                out_vis.append(data)
            for j in range(batch_size):  #this can be optimized
                label_vis.append([0])
            ...
            out_mp = out_mp.view(out_mp.size(0), -1)
   
            ad_out = ad_net(grl(feature)) #output of dann's adversarial network

            loss_bag = loss_fn(output_batch,labels_batch,current_epoch=params.current_epoch, params=params)
            loss_ad = nn.BCELoss()(ad_out.view(-1),ad_target.view(-1))

            trade_off = 0.1
            loss = loss_bag['ls_CE']+ loss_ad * trade_off

            #print(output_batch.data.size())
            #print(labels_batch.data.size())            
    ...
    'ls_all':ls_all_mean_train,'ls_CE':ls_ce_mean_train,'ls_BCE':ls_bce_mean_train}
    print(metrics_mean)
...
def evaluate(model, loss_fn, dataloader, metrics, params,logger):
    ...
    for data_batch, labels_batch in dataloader:
        ...
        output_batch,_,_ = model(data_batch.float())
...
def train_and_evaluate(model,ad_net,grl, ad_net_mp, grl_mp ,train_dataloader, val_dataloader, optimizer,
    ...
    for epoch in range(params.start_epoch,params.num_epochs):
        ...
        train_metrics,train_confusion_meter = train(model,ad_net, grl,ad_net_mp,grl_mp, optimizer, loss_fn, train_dataloader, metrics, params,logger)
...
if __name__ == '__main__':
    ...
    port,env = 8098,params.model_version
    ...
    if 'HCN' in params.model_version:
        ...
        grl_mp = HCN.GRL()
        #grl_m = HCN.GRL()
        # 3D online  320  n2b 240
        ad_net = HCN.AdversarialNetwork(512) #params depend on the size of output feature map
        ad_net_mp = HCN.AdversarialNetwork(1024) #adjustable
        ...
        if params.data_parallel:
            ...
            ad_net_mp = ad_net_mp.cuda(params.gpu_id)
    ...
    para_dic = [{'params':filter(lambda p: p.requires_grad, model.parameters())},{'params':filter(lambda p: p.requires_grad, ad_net.parameters()),'lr':1e-3}
    ,{'params':filter(lambda p: p.requires_grad, ad_net_mp.parameters()),'lr':1e-3}]
    #print(para_list)
    ...
    if params.mode == 'train' or params.mode == 'load_train':
        ...
        train_and_evaluate(model,ad_net, grl,ad_net_mp, grl_mp, train_dl, test_dl, optimizer, loss_fn, metrics, params,
