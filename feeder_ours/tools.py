...
def random_get_frame(data_numpy,valid_frame_num,p_interval,window):
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
    ## valid1: by estimated from data
    # valid_frame = (data_numpy != 0).sum(axis=3).sum(axis=2).sum(axis=0) > 0  # not all person,joints,ordinate is zero(must used be for normalization, , as validate frame
    ...
    average_duration = valid_size//window
    if average_duration < 2:
        ...
        offsets = np.multiply(list(range(window)),average_duration) + randint(average_duration, size=window)
